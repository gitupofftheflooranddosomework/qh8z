...
app.get('/healthz', async (_req, res) => {
  try {
    ...
    res.json({ ok: true, service: 'qh8z', shlinkConfigured: Boolean(config.shlinkApiKey), billingEnabled: billingEnabled() });
  } catch {
...
app.get('/api/config', (_req, res) => {
  res.json({ brand: 'QH8Z', publicShortBaseUrl: config.publicShortBaseUrl, allowSignup: config.allowSignup, billingEnabled: billingEnabled(), supportEmail: config.supportEmail, plans });
});
...
app.post('/api/auth/register', authLimiter, async (req, res, next) => {
  try {
    ...
    const result = await pool.query('INSERT INTO users(id,email,password_hash,name,is_admin) VALUES($1,$2,$3,$4,$5) RETURNING id,email,name,plan,is_admin,created_at', [id, email, passwordHash, name, isAdmin]);
...
app.post('/api/account/password', requireUser, authLimiter, async (req, res, next) => {
  try {
    ...
    if (!validPassword(newPassword)) return res.status(400).json({ error: 'weak_password', message: 'Use at least 10 characters and no more than 72 UTF-8 bytes.' });
...
app.post('/api/links', requireUser, writeLimiter, async (req, res, next) => {
  try {
    ...
    try { longUrl = normalizeHttpUrl(req.body.longUrl); customSlug = normalizeSlug(req.body.customSlug); }
...
app.patch('/api/links/:id', requireUser, writeLimiter, loadOwnedLink, async (req, res, next) => {
  try {
    let longUrl;
    try { longUrl = normalizeHttpUrl(req.body.longUrl ?? req.link.long_url); } catch (error) { return res.status(400).json({ error: 'invalid_link', message: error.message }); }
...
app.post('/api/billing/checkout', requireUser, writeLimiter, async (req, res, next) => { try { const session = await createCheckout(req.user); res.json({ url: session.url }); } catch (error) { next(error); } });
...
app.use((error, _req, res, _next) => {
  ...
  res.status(status).json({ error: status === 500 ? 'internal_error' : 'request_failed', message: status < 500 ? error.message : 'Something went wrong' });
});

await migrate();
await cleanupExpiredSessions();
setInterval(() => cleanupExpiredSessions().catch(console.error), 6 * 60 * 60_000).unref();
app.listen(config.port, '0.0.0.0', () => console.log(`QH8Z listening on :${config.port} (${config.env})`));
