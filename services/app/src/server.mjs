...
app.post('/api/auth/register', authLimiter, async (req, res, next) => {
  try {
    ...
    if (isAdminEmail && !secureEqual(req.headers['x-qh8z-admin-bootstrap'], config.adminBootstrapSecret)) {
      return res.status(403).json({ error: 'admin_bootstrap_required', message: 'This email is reserved for the QH8Z administrator.' });
    }
